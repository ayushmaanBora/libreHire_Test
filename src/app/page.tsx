        ...
        {languages.map((lang) => (
          <div key={lang.name} className="flex items-center gap-1.5">
            ...
            <span className="text-[11px] font-mono text-gray-600">
              {lang.name} <span className="text-gray-400">{Math.round(lang.percentage)}%</span>
            </span>
          </div>
        ))}
      </div>
        ...
        <div className="flex items-start gap-5 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="flex gap-4 mt-2">
              {[
                { label: 'followers', val: profile.followers?.toLocaleString() || 0 },
                { label: 'repos', val: profile.own_repos || profile.public_repos || 0 },
                { label: 'stars', val: profile.stars?.toLocaleString() || 0 },
              ].map(({ label, val }) => (
...
export default function Home() {
  ...
  const handleHunt = async (e: React.FormEvent) => {
    ...
    if (!config.githubToken) {
      setError("ERROR: GITHUB PAT IS REQUIRED. Click 'Configure Engine' first.");
      return;
    }
      ...
      <main className="max-w-4xl mx-auto px-6 mt-16">
        ...
        {activeView === "search" && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {/* SPLIT SEARCH BAR (Updated) */}
            <form onSubmit={handleHunt} className="flex flex-col md:flex-row mb-16 border-4 border-black">
              <input
                type="text"
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder="kernel engineers"
                className="flex-1 text-2xl md:text-3xl font-bold bg-transparent outline-none p-4 placeholder:text-gray-300"
                autoFocus
              />
              <div className="flex border-t-4 md:border-t-0 md:border-l-4 border-black w-full md:w-auto">
                <span className="p-4 bg-gray-100 font-mono font-bold uppercase text-xs flex items-center border-r-4 border-black">
                  IN
                </span>
                <input
                  type="text"
                  ...
                  className="w-full md:w-48 text-xl md:text-2xl font-bold bg-transparent outline-none p-4 placeholder:text-gray-300"
                />
                <button
                  type="submit"
                  disabled={loading}
                  className="bg-black text-white px-8 py-4 font-mono font-bold tracking-widest hover:bg-gray-800 disabled:opacity-50 transition-colors uppercase"
                >
                  {loading ? "..." : "HUNT"}
                </button>
              </div>
            ...
            {/* ERROR */}
            {error && (
              <div className="border-4 border-red-500 bg-white text-red-600 p-6 font-mono text-sm uppercase font-bold text-left tracking-widest mb-12">
                {error}
              </div>
            )}
            ...
            <div className="space-y-16">
              {results.map((profile, idx) => (
                <DeveloperCard key={profile.handle || idx} profile={profile} rank={idx + 1} />
              ))}
            </div>
